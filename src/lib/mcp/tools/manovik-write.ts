...
const FORMAT_PROMPT: Record<string, string> = {
  article:
    "Write a structured long-form article with H2/H3 headings, scannable paragraphs and a conclusion.",
  blog: "Write an engaging blog post: hook opening, subheadings, concrete examples, short paragraphs.",
  landing:
    "Write landing-page copy: headline, subheadline, 3-5 benefit blocks, social-proof line, and a CTA.",
  email: "Write an email: subject line, preview text, tight body, single clear call to action.",
  social:
    "Write short social posts: punchy, no fluff, platform-appropriate length, no hashtag spam.",
...
export default defineTool({
  ...
  inputSchema: {
    topic: z
      ...
      .min(1)
      .max(8000)
      .describe("What to write about, including any key facts to include."),
    format: z
    ...
    tone: z
      ...
      .max(80)
      .optional()
      .describe("Desired tone, e.g. 'confident, technical, no hype'."),
    audience: z
      .string()
      .trim()
      .max(160)
      .optional()
      .describe("Who is reading, e.g. 'senior backend engineers'."),
    words: z.number().int().min(50).max(4000).optional().describe("Approximate target word count."),
    keywords: z
      .array(z.string().max(60))
      .optional()
      .describe("SEO keywords to weave in naturally."),
    language: z
      .string()
      .trim()
      .max(40)
      .optional()
      .describe("Output language. Defaults to the input language."),
  },
  ...
  handler: async ({ topic, format, tone, audience, words, keywords, language }) => {
    try {
      ...
      return {
        ...
        structuredContent: { content: text, format, model, tier },
      };
    } catch (err) {
