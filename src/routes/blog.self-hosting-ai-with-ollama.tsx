  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: "keywords",
        content:
          "self-hosting AI, Ollama, sovereign AI, local AI coding, private AI agent, Docker AI",
      },
      { property: "og:title", content: "Self-Hosting AI Agents with Ollama and Docker" },
      {
        property: "og:description",
        content:
          "Deploy MANOVIK AI locally with Docker + Ollama for a private, sovereign coding agent.",
      },
      ...
      { name: "twitter:title", content: "Self-Hosting AI Agents with Ollama" },
      {
        ...
        content: "Deploy MANOVIK + Ollama locally for sovereign AI coding.",
      },
    ],
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16 text-foreground">
      <nav className="text-sm text-muted-foreground mb-6">
        ...
        / <span>Blog</span> / <span>Self-hosting AI with Ollama</span>
      </nav>
      ...
      <article className="prose prose-invert max-w-none">
        ...
        <p>
          ...
          (finance, healthcare, defense, government) or any team handling proprietary code, that's a
          non-starter. Self-hosting with <strong>Ollama</strong> + <strong>Docker</strong> gives you
          GPT-class coding assistance with zero outbound calls.
        </p>
        ...
        <pre className="rounded-lg bg-muted p-4 overflow-x-auto">
          <code>{`# One-line install on Linux/macOS
...
ollama --version`}</code>
        ...
        <pre className="rounded-lg bg-muted p-4 overflow-x-auto">
          <code>{`# Strong general coder, ~4.7 GB
...
ollama run qwen2.5-coder:7b "Write a Python function to reverse a linked list"`}</code>
        </pre>

        <h2 className="text-2xl font-semibold mt-10 mb-3">Step 3 — Run Ollama in Docker</h2>
        <p>
          Create a <code>docker-compose.yml</code>:
        </p>
        <pre className="rounded-lg bg-muted p-4 overflow-x-auto">
          <code>{`services:
  ollama:
...
docker exec -it $(docker ps -qf name=ollama) ollama pull qwen2.5-coder:7b`}</code>
        </pre>
        ...
        <pre className="rounded-lg bg-muted p-4 overflow-x-auto">
          <code>{`MANOVIK_MODEL_PROVIDER=ollama
MANOVIK_MODEL_ENDPOINT=http://localhost:11434
MANOVIK_MODEL_NAME=qwen2.5-coder:7b`}</code>
        </pre>
        ...
        <pre className="rounded-lg bg-muted p-4 overflow-x-auto">
          <code>{`# Block outbound while testing
sudo iptables -A OUTPUT -p tcp --dport 443 -j REJECT
# Use MANOVIK normally; if it still works, you're sovereign.
sudo iptables -D OUTPUT -p tcp --dport 443 -j REJECT`}</code>
        </pre>
        ...
        <ul className="list-disc pl-6 space-y-1">
          ...
          <li>
            Use <code>OLLAMA_NUM_PARALLEL=4</code> to serve multiple developers at once.
          </li>
          <li>
            Mount the <code>ollama-data</code> volume on fast NVMe — model load time dominates cold
            starts.
          </li>
          <li>
            For 30B+ models, pin to a dedicated GPU node and use <code>OLLAMA_KEEP_ALIVE=-1</code>.
          </li>
        </ul>

        <h2 className="text-2xl font-semibold mt-10 mb-3">Next steps</h2>
        <p>
          You now have a fully sovereign AI coding agent. Pair it with MANOVIK's per-language memory
          and self-update scheduler for an offline assistant that gets smarter without ever phoning
          home.{" "}
