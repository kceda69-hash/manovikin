...
import { AUTH_TEMPLATE_SCHEMAS, type AuthTemplateName } from "@/lib/email-templates/schemas";
...
// The templates take heterogeneous props, so the map is indexed loosely and
// narrowed to a generic component type at the render call site.
type LooseEmailComponent = React.ComponentType<Record<string, unknown>>;
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // The email action type is in hook data (e.g., "signup", "recovery").
        // The guards below preserve the original control flow: a missing or
        // unknown action type ends in the same 400 "Unknown email type" branch.
        const hookData = payload.data ?? {};
        const emailType = hookData.action_type;
        console.log("Received auth event", {
          emailType,
          email_redacted: redactEmail(hookData.email),
          run_id,
        });
        if (!emailType) {
        ...
        const EmailTemplate = EMAIL_TEMPLATES[
          emailType as keyof typeof EMAIL_TEMPLATES
        ] as unknown as LooseEmailComponent | undefined;
        if (!EmailTemplate) {
          console.error("Unknown email type", { emailType, run_id });
          return Response.json({ error: `Unknown email type: ${emailType}` }, { status: 400 });
        }

        // Build template props from hook data (HookData structure)
        const templateProps = {
          siteName: SITE_NAME,
          siteUrl: `https://${ROOT_DOMAIN}`,
          recipient: hookData.email,
          confirmationUrl: hookData.url,
          token: hookData.token,
          email: hookData.email,
          oldEmail: hookData.old_email,
          newEmail: hookData.new_email,
        };
        ...
        if (schema) {
          ...
          if (!parsed.success) {
            ...
            return Response.json({ error: "Invalid template variables" }, { status: 400 });
          }
        }
        ...
        await supabase.from("email_send_log").insert({
          ...
          recipient_email: hookData.email,
          status: "pending",
        });
